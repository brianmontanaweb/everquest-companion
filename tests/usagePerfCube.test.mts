...
import {
  ...
  telemetryRoute,
  telemetryTables,
} from '../scripts/devTelemetryStack.mjs'
...
const cube = (events: TelemetryEvent[], ctx = CTX): PerfCubeRow[] =>
  rollupBatch(batchOf(events), ctx).perf
...
test('the machine class collapses 27 combinations to seven, and the WEAKER axis sets the tier', () => {
  ...
  assert.equal(
    machineClassOf({ cpuCountBucket: 6, totalMemBucket: 1, gpuVendor: 'nvidia' }),
    'low-dgpu',
  )
  assert.equal(
    machineClassOf({ cpuCountBucket: 1, totalMemBucket: 7, gpuVendor: 'intel' }),
    'low-igpu',
  )
  // 6-11 threads and 12-23 GB is the middle of both ladders.
  assert.equal(
    machineClassOf({ cpuCountBucket: 3, totalMemBucket: 4, gpuVendor: 'amd' }),
    'mid-dgpu',
  )
  assert.equal(
    machineClassOf({ cpuCountBucket: 4, totalMemBucket: 3, gpuVendor: 'intel' }),
    'mid-igpu',
  )
  assert.equal(
    machineClassOf({ cpuCountBucket: 5, totalMemBucket: 5, gpuVendor: 'nvidia' }),
    'high-dgpu',
  )
  assert.equal(
    machineClassOf({ cpuCountBucket: 7, totalMemBucket: 7, gpuVendor: 'intel' }),
    'high-igpu',
  )
})
...
test('UNKNOWN IS A CLASS, NOT A GUESS — a missing or unclassable axis takes the whole box', () => {
  ...
  assert.equal(
    machineClassOf({ cpuCountBucket: 5, totalMemBucket: 5, gpuVendor: 'other' }),
    'unknown',
  )
  assert.equal(
    machineClassOf({ cpuCountBucket: 5, totalMemBucket: 5, gpuVendor: 'unknown' }),
    'unknown',
  )
})
...
test('THIS BATCH SNAPSHOT WINS, AND THE INSTALL ROW IS THE FALLBACK', () => {
  const stored = {
    ...CTX,
    perf: { machineClass: 'low-igpu' as const, windowMode: 'windowed' as const },
  }
  ...
  const fresh = cube(
    [snapshot({ eqWindowMode: 'windowed', gpuVendor: 'intel' }), heartbeat()],
    stored,
  )[0]
...
test('THE LOCAL STACK PROVES THE RESOLUTION: a snapshot launch, then heartbeat-only batches', () => {
  ...
  assert.equal(
    telemetryRoute(state, Buffer.from(JSON.stringify(batchOf([snapshot(), heartbeat()], id))), now)
      .status,
    202,
  )
  ...
  assert.equal(
    telemetryRoute(state, Buffer.from(JSON.stringify(batchOf([heartbeat()], id))), now).status,
    202,
  )
...
test('THREE CUTS OF ONE POPULATION, each with its own denominator', () => {
  ...
  for (const cut of [p.byWindowMode, p.byMachineClass, p.byLocked]) {
    assert.equal(
      cut.reduce((sum, s) => sum + s.reports, 0),
      200,
    )
    assert.equal(
      cut.reduce((sum, s) => sum + s.stalls, 0),
      72,
    )
  }
