...
const YOU = source('you', 'You', 'you', [cat('melee', [skill('Melee', { total: 1000, hits: 50, max: 60 })])])
...
function pet(id: string, name: string, total: number, crits: number): SourceView {
  const category = { ...cat('melee', [skill('Melee', { total, hits: 40, crits, max: 200 })]), crits }
...
test('meterSources (combine on, no self) still deprioritizes a crit-less pet', () => {
  ...
  assert.deepEqual(rows.map((r) => r.id), ['pet:2', 'pet:1'])
})
