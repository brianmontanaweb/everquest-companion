...
test('the redactor over the rest of the nasty catalogue', () => {
  const table: [string, string, string][] = [
    ...
    [
      'nothing to redact is left completely alone',
...
test('normalizeFrameFile keeps the bundle path and nothing above it', () => {
  const table: [string, string | null][] = [
    [
      'C:\\Users\\jmoye\\AppData\\Local\\Programs\\eqc\\resources\\app.asar\\out\\main\\index.js',
      'out/main/index.js',
    ],
    [
      'file:///C:/Users/jmoye/dev/eqc/out/renderer/assets/index-a1b2.js',
      'out/renderer/assets/index-a1b2.js',
    ],
...
test('parseStackFrames takes app frames, skips the rest, and caps at ten', () => {
  ...
  const many = Array.from(
    { length: 40 },
    (_, i) => `    at f${String(i)} (out/main/a.js:${String(i + 1)}:1)`,
  ).join('\n')
...
test('the fingerprint groups by name and top frames, and ignores the varying parts', () => {
  ...
  assert.notEqual(
    errorFingerprint('TypeError', a),
    errorFingerprint('TypeError', [frame('out/main/z.js', 10), ...a.slice(1)]),
  )
...
test('ADVERSARIAL: free text is refused in every field', () => {
  ...
  refused(
    sample({
      redactedMessage: '[Sat Aug 01 13:00:28 2026] You slash a rat for 12 points of damage.',
    }),
    'redactedMessage',
  ...
  for (const poison of [`${ESC}[2J`, `${ESC}]0;pwned${BEL}`, ch(0x00), ch(0x202e), '\n', '\t']) {
    refused(
      sample({ redactedMessage: `broke${poison}` }),
      'redactedMessage',
  ...
  refused(
    sample({ errorName: 'the app broke while zoning into Plane of Sky' }),
    'errorName',
    'prose',
  )
  ...
  refused(
    sample({ breadcrumbs: [{ kind: 'a rat hit you', offsetMs: 0 }] }),
    'breadcrumbs[0].kind',
    'a log LINE as a crumb kind',
  )
})
...
test('ADVERSARIAL: the numeric fields are bounded, and a smuggled field never survives', () => {
  refused(
    sample({
      frames: [{ file: 'out/main/a.js', line: MAX_FRAME_POSITION_WIRE + 1, col: 1, func: 'f' }],
    }),
    'frames[0].line',
    'an absurd line',
  )
  refused(
    sample({ frames: [{ file: 'out/main/a.js', line: -1, col: 1, func: 'f' }] }),
    'frames[0].line',
    'a negative line',
  )
  refused(
    sample({ sessionAgeBucket: SESSION_AGE_MS_EDGES.length + 1 }),
    'sessionAgeBucket',
    'a bucket past the edges',
  )
  refused(sample({ count: -1 }), 'count', 'a negative count')
  refused(
    sample({ breadcrumbs: [{ kind: 'damage', offsetMs: 60 * 60_000 }] }),
    'breadcrumbs[0].offsetMs',
    'an offset past the cap',
  )
  ...
  const fatFrame = validateTelemetryEvent(
    sample({
      frames: [
        { file: 'out/main/a.js', line: 1, col: 1, func: 'f', source: 'You slash a rat.' } as never,
      ],
    }),
  )
