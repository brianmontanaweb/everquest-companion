...
async function pickOtherZone(page: Page, current: string): Promise<string | null> {
  ...
  for (const row of rows) {
    const stem = await row.evaluate(
      (el) => (el as HTMLElement).innerText.trim().split('\n').pop()?.trim() ?? '',
    )
...
async function enterZone(page: Page, log: FixtureLog, zone: string): Promise<boolean> {
  ...
  const seen = await settle(
    async () => (await snapshot(page)).zone,
    (z) => z === zone,
    {
      timeoutMs: 30_000,
    },
...
async function stepDefault(page: Page, autoZone: string): Promise<void> {
  check(
    'a fresh install opens the Maps tab FOLLOWING the character',
    (await modeOf(page)) === 'follow',
  )
  check('…and states so exactly once', (await countOf(page, MODE)) === 1)
  check(
    '…with no Current zone button, because following is already what it is doing',
    (await countOf(page, CURRENT)) === 0,
  )
  check(
...
async function stepPinSurvivesTabs(page: Page, picked: string): Promise<void> {
  ...
  check(
    'THE PICKED MAP SURVIVES LEAVING AND RETURNING TO THE TAB',
    after === picked,
    `${String(after)} vs ${picked}`,
  )
...
async function stepPinnedIgnoresZoning(page: Page, log: FixtureLog, picked: string): Promise<void> {
  if (
    !check(
      'the log states a new zone (East Freeport) while a map is pinned',
      await enterZone(page, log, 'East Freeport'),
    )
  ) {
    return
  }
  ...
  check(
    'A PINNED MAP DOES NOT SNAP TO THE ZONE YOU JUST WALKED INTO',
    settled === picked,
    `${String(settled)} vs ${picked}`,
  )
...
async function stepCurrentZoneResumes(page: Page, log: FixtureLog): Promise<void> {
  ...
  check(
    'CURRENT ZONE SNAPS TO THE ZONE THE CHARACTER IS IN',
    snapped === 'freporte',
    String(snapped),
  )
  check('…and hands the choice back to the log', (await modeOf(page)) === 'follow')
  check(
    '…with the button gone, because there is no pin left to end',
    (await countOf(page, CURRENT)) === 0,
  )
  const stored = await settle(
    () => storedOf(page),
  ...
  check(
    `the resumed follow is stored too, not merely un-pinned`,
    stored.mode === 'follow',
  ...
  if (
    !check(
      'the log states another zone (The Greater Faydark)',
      await enterZone(page, log, 'The Greater Faydark'),
    )
  ) {
    return
  }
...
async function stepSurvivesRestart(page: Page, pinned: string): Promise<void> {
  ...
  check(
    '…and it is still pinned, so the next zone line will not take it either',
    (await modeOf(page)) === 'pinned',
  )
  ...
  note(
    `the replayed log leaves the character in "${snap.zone ?? '(none)'}" while the map holds "${pinned}"`,
  )
}
...
async function firstLaunch(page: Page, log: FixtureLog): Promise<string | null> {
  ...
  if (picked == null) {
    note(
      'this machine offers no second zone to pick (no EQ map packs) — the pin assertions are skipped',
    )
    return null
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (pinned == null)
      note('nothing was pinned in launch 1 — the restart half has no subject and is skipped')
