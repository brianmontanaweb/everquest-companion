...
function castChainMez(log: FixtureLog): void {
  ...
  log.appendAt(
    ...
    'a turmoil toad has been mesmerized.',
    'a scareling has been mesmerized.',
  )
}
...
async function stepDefaultOff(page: Page, app: ElectronApplication): Promise<void> {
  ...
  const open = {
    buffs: await windowsOfKind(app, 'buffs'),
    debuffs: await windowsOfKind(app, 'debuffs'),
  }
  check(
...
async function stepOpenAndChrome(
  page: Page,
  ...
  const surface = await settle(
    () => countOf(o, `[data-testid="${s.testid}"]`),
    (n) => n === 1,
    {
      timeoutMs: 20_000,
    },
...
async function stepIndependentBounds(
  ...
  check(
    'moving the debuffs window moves the debuffs window',
    after.debuffs.x === moved.x,
    JSON.stringify(after.debuffs),
  )
  check(
  ...
  const cfgOf = (o: Page): Promise<Bounds | undefined> =>
    o.evaluate(() =>
      (
        window as unknown as { eqOverlay: { getConfig: () => Promise<{ bounds?: Bounds }> } }
      ).eqOverlay
        .getConfig()
        .then((c) => c.bounds),
    )
  ...
  const savedBuffs = await settle(
    () => cfgOf(buffsOverlay),
    (b) => b?.x === buffsWant.x,
    { timeoutMs: 15_000 },
  )
  const savedDebuffs = await settle(
    () => cfgOf(debuffsOverlay),
    (b) => b?.x === debuffsWant.x,
    { timeoutMs: 15_000 },
  )
  check(
...
async function stepChainMez(
  overlay: Page,
  buffsOverlay: Page | null,
  log: FixtureLog,
): Promise<void> {
  castChainMez(log)
  ...
  if (!check('one AE mez cast raises a row PER ENEMY', cc.length === 2, JSON.stringify(seen)))
    return
  ...
  check(
    '…and each counts DOWN from the stated duration',
    cc.every((r) => r.mode === 'countdown'),
    JSON.stringify(cc),
  )
  check(
  ...
  check(
    '…with a receding bar beside it',
    (await countOf(overlay, '[data-testid="buff-timer-fill"]')) >= 2,
  )
  ...
  const blocked = await settle(
    () => groups(overlay),
    (g) => g.some((x) => x.includes('a turmoil toad')),
    {
      timeoutMs: 15_000,
    },
  )
  check(
    'grouping by target raises a heading per enemy',
    blocked.some((x) => x.includes('a scareling')),
  ...
  check(
    '…and switching back takes the headings away again',
    flat.length === 0,
    JSON.stringify(flat),
  )
...
async function stepBreakClearsOneTarget(overlay: Page, log: FixtureLog): Promise<void> {
  ...
  const named = await targets(overlay)
  check(
    'a break line clears ONLY its own target',
    !named.some((x) => x.includes('a scareling')),
    JSON.stringify(named),
  )
  check(
 ...
 */
async function stepDropFlash(
  overlay: Page,
  debuffsOverlay: Page | null,
  log: FixtureLog,
): Promise<void> {
  ...
  const fresh = (r: { name: string; time: string }): boolean =>
    r.name === 'Valor' && r.time !== '0s'
  ...
  if (
    !check('a self buff you cast raises a row of your own', valor !== undefined, JSON.stringify(up))
  ) {
    return
  }
  ...
  check(
    '…counting DOWN from the duration spells.json states',
    valor?.mode === 'countdown',
  ...
  const after = await settle(
    () => rows(overlay),
    (r) => !r.some((x) => x.name === 'Valor'),
    { timeoutMs: 30_000 },
  )
  check(
    'a wears-off message clears the buff it names',
    !after.some((r) => r.name === 'Valor'),
  ...
  check(
    '…and the overlay FLASHES that a positive spell dropped',
    flashed.some((t) => t.includes('Valor')),
  ...
  check(
    '…the self drop naming just the spell',
    flashed.includes('Valor dropped'),
    JSON.stringify(flashed),
  )
}
...
async function stepCloseOne(
  ...
  check(
    `…while the ${other} window is untouched and still open`,
    state[other] === true,
    JSON.stringify(state),
  )
...
async function stepCloseRest(page: Page, app: ElectronApplication, kind: TimerKind): Promise<void> {
  ...
  check(
    '…and both timer overlays end the run closed',
    state.buffs === false && state.debuffs === false,
    JSON.stringify(state),
  )
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const [kind, o] of [
      ['buffs', buffsOverlay] as const,
      ['debuffs', debuffsOverlay] as const,
    ]) {
      if (!o) continue
      o.on('console', (m) => {
    ...
    if (buffsOverlay && debuffsOverlay)
      await stepIndependentBounds(app, buffsOverlay, debuffsOverlay)
