...
async function inEditor(page: Page, body: () => Promise<void>): Promise<void> {
  ...
  await settle(
    () => countOf(page, '[data-testid="alert-dialog"]'),
    (n) => n === 0,
    { timeoutMs: 10_000 },
  )
}
...
async function stepTurnItOn(app: ElectronApplication, page: Page): Promise<Page | null> {
  ...
  check(
    '…and Preferences agrees: the switch is OFF, matching the window that never opened',
    off === false,
    String(off),
  )
  await page.click('[data-testid="pref-banner-enabled"] input')
  const banner = await settle(
    () => findBannerWindow(app),
    (w) => w !== null,
    { timeoutMs: 30_000 },
  )
...
async function stepIntroduction(banner: Page): Promise<void> {
  ...
  if (
    !check(
      'a freshly enabled banner INTRODUCES itself (one line, unprompted)',
      lines.length === 1,
  ...
  check(
    '…naming the per-alert marking that reaches it',
    lines[0].includes('Show on screen'),
    lines[0],
  )
...
async function stepControlsAppear(page: Page): Promise<void> {
  ...
  const toggles = await settle(
    () => countOf(page, ROW_TOGGLE),
    (n) => n > 0,
    { timeoutMs: 15_000 },
  )
  check(
    'with the overlay on, every row offers an on-screen toggle',
    toggles > 0,
    `${String(toggles)} toggle(s)`,
  )
  const on = await page.evaluate(
    (sel) => document.querySelector(sel)?.getAttribute('data-on'),
    ROW_TOGGLE,
  )
  check(
    '…starting ON, because an alert written before this feature existed still shows',
    on === 'true',
    String(on),
  )
  await inEditor(page, async () => {
    ...
    check(
      '…and no longer the note about a switch that is now on',
      (await countOf(page, '[data-testid="alert-banner-off-note"]')) === 0,
    )
  })
}
...
async function stepFiringRendersALine(page: Page, banner: Page): Promise<string> {
  ...
  if (
    !check(
      'pressing ▶ on a row renders a line on the banner',
      lines.length === 1,
      `${String(lines.length)} line(s)`,
    )
  ) {
...
async function stepTextIsCentered(banner: Page): Promise<void> {
  const geom = await banner.evaluate(
    (sel: string[]) => {
  ...
  check(
    'the banner text is CENTRE-aligned in its line',
    geom?.align === 'center',
    JSON.stringify(geom),
  )
  check(
...
async function stepPerAlertSwitchHides(page: Page, banner: Page): Promise<void> {
  ...
  const after = await settleStable(() => lineTexts(banner), {
    timeoutMs: 8_000,
    stable: 5,
    pollMs: 150,
  })
  ...
  await page.evaluate(async () => {
    const eq = (
      window as unknown as {
        eq: {
          getAlertPrefs: () => Promise<Record<string, unknown>>
          setAlertPrefs: (p: unknown) => Promise<unknown>
        }
      }
    ).eq
