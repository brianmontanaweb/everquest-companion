...
const search = (
  q: string,
  ctx = TRIO,
  cap = UNLOCK_SEARCH_CAP,
): ReturnType<typeof searchUnlockSpells> =>
  searchUnlockSpells(
    SPELLS,
    tokenizeSpellQuery(q),
    { classes: [...ctx.classes], currentLevel: ctx.currentLevel },
    cap,
  )
...
test('U2 rows sort by the LOWEST MATCHING level, then name', () => {
  ...
  assert.deepEqual(
    rows.map((r) => r.name),
    ['Superior Healing', 'Celestial Healing'],
  )
  assert.deepEqual(
    rows.map((r) => r.level),
    [27, 28],
...
test('U2c a name the wiki carries twice is ONE row — a bookkeeping artefact never doubles a result', () => {
  ...
  assert.deepEqual(
    rows[0].levels?.map(classLevelLabel),
    ['CLR 29', 'PAL 39'],
    'both statements survive the fold',
  )
})
...
test('U4 the owner query answers over the shipped dataset, and every row earns its place', () => {
  ...
  const { rows, matched } = searchUnlockSpells(
    data.spells,
    tokenizeSpellQuery('27-28 cleric shaman'),
    {
      classes: ['CLR', 'PAL', 'ENC'],
      currentLevel: 30,
    },
...
test('U4b a NAME finds its spell with every class level on it, and the rows keep their worth', () => {
  ...
  assert.deepEqual(
    row.levels?.map(classLevelLabel),
    ['CLR 39'],
    'it states the level the DB places it at',
  )
...
test('U4c every shipped row carries the search surface the filter reads', () => {
  ...
  for (const s of data.spells) {
    ...
    assert.equal(
      ...
      `${s.name}: the surface must be lowercased`,
    )
    assert.ok(
      searchUnlockSpells([s], tokenizeSpellQuery(s.name), { classes: [], currentLevel: null })
        .matched === 1,
