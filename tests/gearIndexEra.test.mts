...
test('a derived out-of-era row is hidden by the filter and its chip names the edge', () => {
  ...
  assert.equal(
    chip.label,
    'out of era',
    'the derivation names no expansion, so neither may the chip',
  )
  assert.match(
    chip.tooltip,
    /Small Breastplate Mold/,
    'the tooltip must name the edge that decided',
  )
  ...
  assert.match(
    eraChip(smb)?.tooltip ?? '',
    /Scaled Mystic Armor Quests, a quest that starts in East Cabilis/,
  )
})
...
test('SILVER FULL BREASTPLATE: the set page the item corpus could not hold now decides it', () => {
  ...
  assert.match(
    ...
    'the tooltip must name the page that decided',
  )
...
test("LIFE'S GUARD: the dropper edge overrules the zone, and the chip names the mob", () => {
  ...
  assert.equal(
    guard.eraTag,
    'Classic',
    'its own page claims Classic — which is what gets overruled',
  )
  ...
  assert.match(
    chip?.tooltip ?? '',
    /Agent of Innoruuk/,
    'the tooltip must name the mob that decided',
  )
...
test('layer 3 is counted in the build census, and only its ONE definitive edge overrules a row', () => {
  ...
  assert.ok(
    index.stats.eraDerivedRows >= 2000,
    `only ${String(index.stats.eraDerivedRows)} derived rows`,
  )
  ...
  for (const row of rows) {
    ...
    if (derived.definitive ?? false) {
      ...
      assert.equal(
        derived.basis,
        'drop-mob',
        `${row.name} claims a definitive ${derived.basis} edge`,
      )
    } else {
      assert.equal(
        row.eraTag,
        undefined,
        `${row.name} carries BOTH a banner and a non-definitive derivation`,
      )
    }
