...
test('R2 a different class is a different ladder, and an unknown spell says so', () => {
  ...
  assert.deepEqual(replacedBy('Minor Healing', 'WIZ'), {
    replaces: null,
    replacedBy: null,
    line: null,
  })
  // Nor is something no line carries.
  assert.deepEqual(replacedBy('Not A Spell At All', 'CLR'), {
    replaces: null,
    replacedBy: null,
    line: null,
  })
...
function checkNeighbours(
  cls: ClassAbbr,
  line: { name: string; members: { name: string; level: number }[] },
): void {
  ...
  for (const m of line.members) {
    ...
    if (place.replacedBy !== null) {
      assert.ok(
        (at(place.replacedBy) ?? 99) > m.level,
        `${cls} ${m.name} is replaced by ${place.replacedBy}`,
      )
    }
  }
}
...
function checkRepairs(
  cls: ClassAbbr,
  raw: RawLine,
  shipped: Map<string, { members: { name: string; level: number }[] }>,
  ...
  if (!out) {
    ...
    for (const m of members) {
      assert.equal(
        m.inDb,
        false,
        `${cls} ${raw.id} was dropped, so every member must be off-Legends`,
      )
    }
  ...
  for (const m of offLegends)
    assert.ok(!names.has(m.name.toLowerCase()), `${cls} ${raw.id}: ${m.name}`)
  // 2. no twin survived
  ...
  return {
    dropped: offLegends.length,
    deduped: members.length - offLegends.length - out.members.length,
  }
}

test('R5 the four repairs hold against the raw research files', () => {
  const files = readdirSync(RESEARCH).filter(
    (f) => f.startsWith('lines-') && f !== 'lines-merged.json',
  )
...
test('R6 the shipped file covers the thirteen spell-casting classes and nothing else', () => {
  ...
  for (const cls of withLines) {
    for (const line of linesForClass(cls)) {
      ...
      for (const m of line.members) {
        assert.ok(
          m.name.length > 0 && m.level >= 1 && m.level <= 70,
          `${cls} ${m.name}@${String(m.level)}`,
        )
      }
    }
  }
