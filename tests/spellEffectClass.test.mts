...
function names(
  klass: Parameters<typeof effectRoster>[1],
  opts?: Parameters<typeof effectRoster>[2],
): string[] {
...
test('JOS-251 R1b: …and its class breakdown is the research`s, re-derived from the DB', () => {
  const per = (cls: string): string[] =>
    names('charm').filter((n) =>
      RAW.some((s) => s.name === n && new RegExp(`\\*\\s*\\[*${cls}`).test(s.classes ?? '')),
    )
  assert.equal(per('Enchanter').length, 6, 'Charm 11 → Dictate 60')
  assert.equal(per('Druid').length, 7, 'three of these had no stem at all')
  assert.deepEqual(
    per('Shaman'),
    ['Befriend Animal', 'Charm Animals'],
    'both shared with the druid list',
  )
...
test('JOS-251 R1c: the NPC charms are real charms, and are excluded by the CASTABLE gate', () => {
  ...
  assert.deepEqual(
    names('charm', { castableOnly: false }).filter((n) => NPC_CHARMS.includes(n)),
    NPC_CHARMS,
  )
  assert.deepEqual(
    names('charm').filter((n) => NPC_CHARMS.includes(n)),
    [],
  )
})
...
const AUDIT_FALSE_POSITIVES: Record<string, string> = {
  ...
  "Boltran's Animation":
    'a PET SUMMON with a 9,000 ms cast — the `boltran` stem armed a 10.5-second false charm window, the exact foreign-pet adoption the ownership model exists to prevent',
  "Naki's Charm of Pernicity":
    'an ITEM focus effect — a charm is a trinket as well as a spell in this game',
...
test('JOS-251 R2: every false positive the audit found is a non-charm by its effect line', () => {
  for (const [name, why] of Object.entries(AUDIT_FALSE_POSITIVES)) {
    ...
    assert.ok(
      row,
      `spells.json must still carry "${name}" — a table naming a spell that no longer exists is a stale claim, not a passing test`,
    )
...
test('JOS-251 R3: the hold roster is mez ∪ root, and a movement debuff is in NEITHER', () => {
  ...
  assert.ok(
    !spellHasEffect(
      RAW.find((s) => s.name === "Largo's Melodic Binding")!,
      'snare',
    ),
    'AC and attack speed only',
  )
  assert.ok(
    spellHasEffect(
      RAW.find((s) => s.name === "Largo's Assonant Binding")!,
      'snare',
    ),
    '61% at L60',
  )
...
const CC_STEM_FALSE_POSITIVES: Record<string, string> = {
  Ensnare:
    'druid 26 — a pure `Decrease Movement Speed by 40%`, caught by the `ensnar` stem that exists for Ensnaring ROOTS',
...
test('JOS-251 R4b: the anchor is what keeps the near-misses out', () => {
  ...
  assert.equal(
    classifyEffectLine('Add Melee Proc: Stunning Strike'),
    null,
    'adds a proc, does not stun',
  )
  ...
  assert.equal(
    classifyEffectLine('Decrease Hitpoints by 400'),
    null,
    'no opinion is the honest answer',
  )
...
test('JOS-251 R4c: both wiki phrasings of a charm read the same, and a spell can be two things', () => {
  ...
  assert.ok(sirens)
  assert.deepEqual(sirens.effects, [
    'Charm (up to L37)',
    'Decrease Magic Resist by 4 (L27) to 8 (L60)',
  ])
  assert.deepEqual(
    spellEffectClasses(sirens),
    ['charm'],
    'the resist debuff is not a class this module reads',
  )
  assert.equal(
    sirens.instrumentEnhanced,
    'Yes (just the resists debuff)',
    'the bard row, captured verbatim',
  )
})
...
test('JOS-251 R4d: the per-class membership counts, pinned', () => {
  ...
  assert.deepEqual(Object.fromEntries(EFFECT_RULES.map((r) => [r.klass, all(r.klass)])), {
    charm: 23,
    summonPet: 102,
    mez: 16,
    root: 24,
...
test('JOS-318 R9: the HoT class reads BOTH wiki phrasings, and no DoT', () => {
  ...
  assert.equal(
    classifyEffectLine('Increase Hitpoints by 60 per tick'),
    'healOverTime',
    'Flowering Heal',
  )
  assert.equal(
    classifyEffectLine('Increase Current Hit Points by 160 per Tick'),
    'healOverTime',
    'Slugs Healing',
  )
  assert.equal(
    classifyEffectLine('Increase Hitpoints v2 by 300 per tick'),
    'healOverTime',
    'Torpor',
  )
  ...
  assert.equal(
    classifyEffectLine('Increase Hitpoints by 688'),
    null,
    "Kragg's Salve is a direct heal",
  )
  assert.equal(
    classifyEffectLine('Increase Hitpoints by 197 after 4 ticks'),
    null,
    'a delayed lump',
  )
})
...
test('JOS-318 R9b: the roster answers for every HoT the OWNER`S LOG has printed a tick for', () => {
  ...
  const LOGGED: readonly [string, number][] = [
    ['Snails Healing', 455],
    ['Tortoises Healing', 314],
    ['Slugs Healing', 248],
    ['Ethereal Cleansing', 214],
    ['Echoing Light', 93],
    ['Celestial Echo', 57],
    ['Sacred Echo', 57],
    ['Renewing Echo', 48],
    ['Sprouting Heal', 48],
    ['Efflorescing Heal', 35],
    ['Celestial Remedy', 34],
    ['Blossoming Heal', 22],
    ['Flowering Heal', 16],
    ['Budding Heal', 6],
  ...
  assert.equal(
    RAW.some((s) => s.name.startsWith('Harm Touch')),
    false,
...
test('JOS-258 R8: the summon class reads what the spell DOES, not how the wiki FILED it', () => {
  ...
  assert.deepEqual(
    byType.filter((n) => !byEffect.includes(n)),
    [],
    'the effect read is a superset',
  )
...
test('JOS-258 R8b: three heads, and the near-misses the anchor keeps out', () => {
  ...
  assert.equal(
    classifyEffectLine('Summon Skeleton Pet: skel_pet_43_'),
    'summonPet',
    'Minion of Shadows',
  )
  ...
  assert.equal(
    classifyEffectLine('Call Pet'),
    null,
    'Summon Companion moves a pet, it does not make one',
  )
...
test('JOS-258 R8c: the roster gate is INVERTED against the charm one, and on purpose', () => {
  ...
  assert.equal(
    effectRoster(RAW, 'summonPet').size,
    1,
    'the default gate keeps only Flaming Sword of Xuzl',
  )
  // The castable gate stays ON: no player prints a cast line for an NPC-only spell.
  assert.deepEqual(
    names('summonPet', { castableOnly: false, targetOnly: false }).filter(
      (n) => !names('summonPet', { targetOnly: false }).includes(n),
    ),
...
test('JOS-251 R4e: the scrape captured an effect list for all but four spells', () => {
  ...
  assert.equal(
    file.withEffects,
    RAW.filter((s) => s.effects?.length).length,
    'the header counts what the rows carry',
  )
...
test('JOS-251 R5: the calm line read two ways — and the two oracles disagree, in both directions', () => {
  ...
  const byMessage = RAW.filter(
    (s) => s.msgCastOnOther != null && CALM_LANDING_MESSAGES.has(s.msgCastOnOther),
  ).map((s) => s.name)
  ...
  assert.deepEqual(byMessage.filter((n) => !byEffect.includes(n)).sort(), [
    'Alliance',
    'Atone',
    'Benevolence',
    'Collaboration',
...
test('JOS-251 R6: classification never writes through the entry it is handed', () => {
  ...
  assert.equal(
    JSON.stringify(RAW),
    before,
    'the imported JSON module is shared by every reader in the process',
  )
})
...
test('JOS-251 R7: THE SEPARATION — the roster comes off the RAW scrape, and the overlay changes nothing', () => {
  ...
  assert.deepEqual(
    [...corrKeys].filter((k) => !rawKeys.has(k)),
    ["solon's bewitching bravura"],
  )
  assert.deepEqual(
    [...rawKeys].filter((k) => !corrKeys.has(k)),
    ["solon's bravura"],
  )
