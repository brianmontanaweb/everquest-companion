...
export const RELEASE_NOTES: readonly ReleaseNote[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      {
        kind: 'fixed',
        text: 'Switching characters no longer replays old alerts and celebrations.',
      },
      {
        kind: 'fixed',
  ...
  },
  {
    ...
    entries: [
      ...
      {
        text: 'The installer runs under Wine and CrossOver instead of dead-ending.',
        fromReport: true,
      },
    ],
  },
...
export function releaseNotesProblems(notes: readonly ReleaseNote[] = RELEASE_NOTES): string[] {
  ...
  notes.forEach((n, i) => {
    if (!VERSION_RE.test(n.version))
      problems.push(`${n.version}: not a plain MAJOR.MINOR.PATCH version`)
    ...
    for (const e of n.entries) {
      ...
      if (e.kind !== undefined && !KINDS.includes(e.kind))
        problems.push(`${n.version}: unknown kind "${e.kind}"`)
    ...
    if (prev && compareVersions(prev.version, n.version) <= 0) {
      problems.push(
        `${n.version} must sort strictly below ${prev.version} - the list is newest first`,
      )
    }
  })
  return problems
