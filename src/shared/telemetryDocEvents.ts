...
export const TELEMETRY_DOC_EVENTS: readonly DocEvent[] = [
  ...
  },
  {
    ...
    fields: [
      {
        name: 'view',
        type: values(TELEMETRY_VIEWS),
        note: 'Which tab. A fixed list of tab names.',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      // stalls would be a promise the code does not keep.
      {
        name: 'parserStalls',
        type: COUNT,
        note: 'Times log reading stalled. Not currently measured — always 0.',
      },
      { name: 'presenceRestarts', type: COUNT, note: 'Times the game-window watcher restarted.' },
      {
        name: 'speechFailures',
        type: COUNT,
        note: 'Times an utterance failed to speak. Downloaded voices only.',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        name: 'fingerprint',
        type: '16 hex characters',
        note: 'A hash used to group identical errors together.',
      },
      {
        name: 'breadcrumbs',
      ...
      {
        name: 'view',
        type: values(TELEMETRY_ERROR_VIEWS),
        note: 'Which tab was open. A fixed list.',
      },
      { name: 'sessionAgeBucket', type: BUCKET, note: 'How long the app had been running.' },
      {
        name: 'mode',
        type: values(TELEMETRY_ERROR_MODES),
        note: 'Was it reading your log history, or following it live.',
      },
      {
        name: 'count',
        type: COUNT,
        note: 'How many times this same error happened since the last report. It stops at a hundred per error per run of the app: something that goes wrong over and over reports itself a hundred times and then goes quiet, so one repeating fault cannot bury everything else.',
      },
    ],
  },
  {
