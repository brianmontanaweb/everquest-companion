...
const EXAMPLE =
  'Paste the line the game printed (“Your Location is 1414.20, -735.55, 12.19”) or just the numbers.'
...
export function parseLoc(text: string): LocParse {
  ...
  if (!nums.every((n) => Number.isFinite(n)))
    return { ok: false, reason: `That reads as no position at all. ${EXAMPLE}` }
  if (nums.length !== 2 && nums.length !== 3) {
    return {
      ok: false,
      reason: `${String(nums.length)} numbers - a /loc is three (north/south, west/east, elevation). ${EXAMPLE}`,
    }
  }
