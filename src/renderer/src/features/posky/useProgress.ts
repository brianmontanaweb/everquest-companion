...
import { useTurnInLedger } from './useTurnInLedger'
import type {
  DerivedEvidence,
  QuestTurnIns,
  TurnInInstants,
...
function useHeldItems(x: {
  ...
  /**
   * WHAT EACH OF THOSE DETECTED TURN-INS ACTUALLY OFFERED (the Sky over-hand-in fix) — read by
   * reconcile.ts as an additive correction on top of the ordinary `need`-per-turn-in subtraction.
   */
...
// useTurnInLedger — split into its own file (useTurnInLedger.ts): where turn-in counts come from,
// when they celebrate, when they are written down, and how a user states one by hand. This file
// was at the measured 400-code-line ceiling; the split is file mass, not scope.
